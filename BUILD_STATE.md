...
- A route guard that returns `<Redirect>` INSTEAD of the navigator leaves the target route with nothing to render into. It blanks, redirects again, and remounts the target on every pass, so any local state on that screen resets and its buttons look dead. Render the navigator always and gate the redirect on the pathname so it cannot re-fire once it has arrived.
- Every screenshot seeded `onboardedAt`, so nothing in the harness ever performed a real first run and the broken tutorial shipped. `test/flow.mjs` now walks the export with no seeded state at all, clicks through all four steps, and checks navigation stops rather than counting it (expo-router does several navigations of its own settling in).
- react-native-web drops the `resizeMode` PROP on the static export. A 128px grain plate painted one untouched tile in the top left corner of every screen and nothing else, which reads as a stray gray box rather than a broken image. Put `resizeMode` in the style object, where both platforms read it.
- A first-run redirect breaks every screenshot: the harness seeds localStorage after its first navigation, so the guard fires and each shot lands on the tutorial. Seed `onboardedAt` with `addInitScript` before the cold navigation and in the seed block for the rest.
- Canned demo content becomes a test fixture whether you meant it to or not. Both browser suites asserted on the lorem card's choice text, so writing real cards broke them. They now read the answers from the card itself.
